...
class MultiheadAttention(nn.Module): 
  ...
  def forward(self, x): 
    ...
    batch_size, num_channels, hidden_size = x.shape
    assert hidden_size % self.num_heads == 0 

    qkv = self.proj_qkv(x)
    query, key, value = qkv.chunk(3, dim = 2)

    dims_per_head = hidden_size // self.num_heads

    # (batch_size, num_heads, channels_per_head, hidden_size)
    query = query.contiguous().view(batch_size, num_channels, self.num_heads, -1)
    query = query.permute(0, 2, 1, 3)
    key   = key.contiguous().view(batch_size,   num_channels, self.num_heads, -1)
    key   = key.permute(0, 2, 1, 3)
    value = value.contiguous().view(batch_size, num_channels, self.num_heads, -1)
    ...
    matching_mat  = torch.bmm(query, key.permute(0, 2, 1)) / math.sqrt(dims_per_head)
    matching_mat  = torch.softmax(matching_mat, dim = -1) # (batch_size * num_heads, num_channels, num_channels) 

    attn_out = torch.bmm(matching_mat, value) # (batch_size * num_heads, num_channels)
    attn_out = attn_out.contiguous().view(batch_size, self.num_heads, num_channels, -1) 
    attn_out = attn_out.permute(0, 2, 1, 3) 
    attn_out = attn_out.contiguous().view(batch_size, num_channels, -1)
    attn_out = self.proj_out(attn_out)
    return attn_out 

class CrossAttention(nn.Module): 
  def __init__(self, num_heads, hidden_size, context_size):
    ...
    self.proj_k = nn.Linear(context_size, hidden_size)
    self.proj_v = nn.Linear(context_size, hidden_size)
  ...
  def forward(self, x, context): 
    ...
    query = self.proj_q(x)
    key = self.proj_k(context)
    value = self.proj_v(context)

    dims_per_head = hidden_size // self.num_heads

    # (batch_size, num_heads, channels_per_head, hidden_size)
    query = query.contiguous().view(batch_size, q_len, self.num_heads, -1)
    query = query.permute(0, 2, 1, 3)
    key   = key.contiguous().view(batch_size, k_len, self.num_heads, -1)
    key   = key.permute(0, 2, 1, 3)
    value = value.contiguous().view(batch_size, k_len, self.num_heads, -1)
    value = value.permute(0, 2, 1, 3)

    # (batch_size * num_heads, channels_per_head, hidden_size)

    query = query.contiguous().view(batch_size * self.num_heads, q_len, -1)
    key   = key.contiguous().view(batch_size * self.num_heads, k_len, -1)
    value = value.contiguous().view(batch_size * self.num_heads, k_len, -1)

    matching_mat = torch.bmm(query, key.permute(0, 2, 1)) / math.sqrt(dims_per_head)
    matching_mat = torch.softmax(matching_mat, dim = -1) # (batch_size * num_heads, q_len, k_len)

    attn_out = torch.bmm(matching_mat, value) # (batch_size * num_heads, q_len, dims_per_head)
    attn_out = attn_out.contiguous().view(batch_size, self.num_heads, q_len, -1) 
    attn_out = attn_out.permute(0, 2, 1, 3) 
    attn_out = attn_out.contiguous().view(batch_size, q_len, -1)
    result = self.proj_out(attn_out)
    return result 


class TransformerBlock(nn.Module):
 def __init__(self, hidden_size, context_size, train_with_context, num_heads, ffn_dim, dropout, activation = 'GeLU'):
   ...
   if self.train_with_context : 
    self.cross_attn = CrossAttention(num_heads = num_heads, hidden_size = hidden_size, context_size = context_size)
   ...
   self.adaln_module = nn.Sequential(
    nn.SiLU(), 
    nn.Linear(hidden_size, (9 if train_with_context else 6) * hidden_size, bias = True)
   )
 def forward(self, x, cond, context = None):
   if self.train_with_context : 
    mean_mha, std_mha, gate_mha, mean_mhca, std_mhca, gate_mhca, mean_mlp, std_mlp, gate_mlp = self.adaln_module(cond).chunk(9, dim = 1)
    x = x + gate_mha.unsqueeze(1)  * self.attn((std_mha.unsqueeze(1) + 1.) * self.norm1(x) + mean_mha.unsqueeze(1))
    x = x + gate_mhca.unsqueeze(1) * self.cross_attn((std_mhca.unsqueeze(1) + 1.) * self.norm2(x) + mean_mhca.unsqueeze(1), context)
    x = x + gate_mlp.unsqueeze(1)  * self.mlp( (std_mlp.unsqueeze(1) + 1.) * self.norm3(x) + mean_mlp.unsqueeze(1))
   else : 
...
class DiffusionTransformer(nn.Module):
  def __init__(
    ...
    self.blocks = nn.ModuleList([
      TransformerBlock(
        ...
        train_with_context = self.train_with_context, 
      )
  ...
  def forward(self, imgs, timesteps, context = None, labels = None):
    """
    ...
    """
    if self.train_with_context : 
      assert context is not None, "In train with context mode and the given context is None"
   # print(imgs.shape, self.patch_embedder(imgs).shape, self.patch_pos_embedding.shape)
    ...
    for block in self.blocks:
      x = block(x, conditional_embedding, context) # (B, T, hidden_size)
