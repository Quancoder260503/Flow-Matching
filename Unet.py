...
class Downsample(nn.Module): 
  def __init__(self, channels, use_conv, dims = 2, out_channels = None):
    ...
    if self.use_conv : 
      self.conv = get_conv_by_dim(dims, self.channels, self.out_channels, \
...
class ResidualBlock(nn.Module): 
  def __init__(self, 
    ...
    self.conv2 = zero_module(get_conv_by_dim(dims, self.out_channels, self.out_channels,\
...
class Unet(nn.Module): 
  def __init__(
    ...
    self.out = nn.Sequential(
      ...
      nn.SiLU(), 
      zero_module(get_conv_by_dim(dims, current_channels, out_channels,\
        kernel_size = 3, padding = 1)), # To reduce the instabilities in the early stage of training 
    )
